...
void dfs_rewrite(int x,int tp)//当前点以及当前点所在重链的顶端
{
    ...
    ctr[x]=tot;//记录以x为根的子树的新编号中最大的那个，子树的范围就是id[x]~ctr[x] 
}
```
...
void change_xtoy()//修改
{
    ...
    while(top[x]!=top[y])//假如不在同一条重链上 
    {
        ...
        change(1,id[top[x]],id[x],z);//修改路径上的点的值 
        x=fa[top[x]];//往上跳 
    }
    if(deep[x]>deep[y])swap(x,y);
    change(1,id[x],id[y],z);//当他们在同一条重链上时，最后修改一下x~y路径上的点 
}
```
...
void getsum_xtoy()//查询，与修改基本相同
{
    ...
    scanf("%d %d",&x,&y);
    ll ans=0;
    while(top[x]!=top[y])  //x与y不在同一条重链上
    {
        if(deep[top[x]]<deep[top[y]])swap(x,y); //将x点改为深度更深的那个点
        ans=(ans+getsum(1,id[top[x]],id[x]))%p; //加上x点到链头的所有点的和，getsum为线段树操作
        x=fa[top[x]];
    }
    if(deep[x]>deep[y])swap(x,y);
...
void change_sontree()
{
    int x,y;
    scanf("%d %d",&x,&y);
    change(1,id[x],ctr[x],y);//因为一颗子树的编号是连续的，所以直接修改即可（ctr前面有） 
}
